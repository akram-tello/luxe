...
export function SaleForm({ clientId }: { clientId: string }) {
  ...
  return (
    <form action={action} className="space-y-3">
      <p className="label">Record sale</p>
      <input name="product" required className="input" placeholder="Product (e.g. Submariner 126610LN)" />
      <div className="grid grid-cols-2 gap-3">
        <input name="reference" className="input" placeholder="Reference" />
        <input name="amount" required type="number" step="0.01" className="input" placeholder="Amount" />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <input name="currency" className="input" placeholder="USD" defaultValue="USD" maxLength={3} />
